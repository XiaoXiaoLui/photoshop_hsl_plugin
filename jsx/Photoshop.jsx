// #include "json.jsx" doesn't work for me, so I just paste the code here.
// Go to bottom of this file to add function.
/*******************************************************************************
...
$._ext_PHXS={
    ...
    getColor: function() {
        //var a = '{"name": "lvgb", "age": 16}';
        //return JSON.lave(a);
        var fdc = app.foregroundColor;
        var bdc = app.backgroundColor;
    ...
    getAllColorSamplers: function() {
        try {
            // this line is ok, but calling methods or getting property of samplers makes cursor turn to windows default arrow shape.
           var samplers = app.activeDocument.colorSamplers;
           var colors = [];
           for (var i = 0; i < samplers.length; i++) {
               colors.push(samplers[i].color);
           }
           
           // this line can correct the cursor shape
           app.backgroundColor.hsb.hue = app.backgroundColor.hsb.hue;
           
           return JSON.lave(colors);
        } catch (e) {
            return "XXX" + JSON.lave(e);
        } finally {
